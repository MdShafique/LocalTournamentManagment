    ...
    <div className={`bg-white rounded-lg shadow-lg p-6 border border-slate-200 relative ${isUpdating ? 'opacity-80' : ''}`}>
      ...
      <div className="flex justify-between items-center mb-6 border-b pb-4">
         ...
         <div className="flex gap-2">
            ...
            {canSwapTeams && (
                <button onClick={handleSwapBattingFirst} disabled={isUpdating} className="p-2 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-2 text-sm font-bold">
      ...
      {match.status === MatchStatus.COMPLETED ? (
           <div className="text-center py-8">
               ...
               <p className="text-emerald-600 font-bold text-lg mb-6">Winner: {match.winnerId === teamA.id ? teamA.name : (match.winnerId === teamB.id ? teamB.name : "Match Tied")}</p>
                    ...
                    <div className={`flex flex-wrap justify-center gap-2 p-2 rounded-lg border transition-colors ${wicketType !== 'NONE' ? 'bg-red-50 border-red-200' : 'bg-slate-50 border-slate-100'}`}>
                        ...
                        {wicketType === 'NONE' ? (
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => setWicketType('NONE')} className="p-1 hover:bg-red-200 rounded-full text-red-600"><XCircle size={16}/></button>
                            </div>
                        )}
                    </div>
                </div>
